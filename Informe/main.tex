...
\section{Documentación de Servicios y Procesos Cliente}

Para soportar las nuevas funcionalidades e integraciones de la 3ra Entrega, se ha extendido el contrato técnico del sistema Sync-Stock. Las peticiones externas son recibidas por el router de Express (actuando como un Protocol Bridge), el cual decodifica y valida las credenciales y el token JWT, evalúa el control de acceso por rol (RBAC) con el middleware \texttt{permitirRoles}, empaqueta el contenido y lo retransmite al Enterprise Service Bus (ESB) por sockets TCP en formato JSON.

A continuación se detallan las operaciones adicionales incorporadas al catálogo de servicios:

\subsection{Registro de Nuevos Productos}
\textbf{Endpoint:} \texttt{POST /api/v1/sales/products} \\
\textbf{Rol Autorizado:} \texttt{Admin} \\
...
{
  "status": "success",
  "mensaje": "Producto registrado y stock inicial sembrado.",
  "producto": {
...
\paragraph{Respuesta de Error - SKU Duplicado (400 Bad Request):}
\begin{verbatim}
{
  "codigoError": "SKU_DUPLICADO",
  "mensaje": "El SKU proporcionado ya está registrado en el sistema.",
  "timestamp": "2026-06-03T19:30:00Z"
}
\end{verbatim}

\subsection{Modificación de Producto}
\textbf{Endpoint:} \texttt{PUT /api/v1/sales/products/:sku} \\
\textbf{Rol Autorizado:} \texttt{Admin} \\
...
\textbf{Endpoint:} \texttt{POST /api/v1/sales/orders/:id/dispatch} \\
\textbf{Rol Autorizado:} \texttt{Admin}, \texttt{ECommerce} \\
...
{
  "status": "success",
  "mensaje": "Orden d3b07384 despachada exitosamente."
}
\end{verbatim}

\paragraph{Respuesta de Error - Estado Inválido (400 Bad Request):}
\begin{verbatim}
{
  "codigoError": "ESTADO_DESPACHO_INVALIDO",
  "mensaje": "Solo se pueden despachar órdenes que estén en estado Pagada.",
  "timestamp": "2026-06-03T19:35:00Z"
}
\end{verbatim}

\subsection{Redirección General a través del Bus de Servicios (GET)}
Para dar cumplimiento estricto con el diseño arquitectónico SOA definido para PescaTotal, se refactorizaron las rutas del módulo de ventas en el router REST del backend. Peticiones de solo lectura como:
\begin{itemize}
    \item \texttt{GET /sales/products} (Catálogo completo de productos)
    \item \texttt{GET /sales/orders} (Historial consolidado de órdenes)
    \item \texttt{GET /sales/reservations} (Listado de reservas web activas)
    \item \texttt{GET /sales/history} (Log de auditoría de movimientos)
\end{itemize}
Ya no realizan llamadas directas de lectura a Prisma en el servidor web Express. En su lugar, emiten una llamada TCP estructurada a través de \texttt{requestSOABus} al microservicio correspondiente. El bus de servicios ESB se encarga de resolver la consulta y retornar la respuesta, manteniendo el desacoplamiento físico de la capa de datos.
...
\begin{enumerate}
    ...
    Para garantizar que no ocurran sobreventas en checkout simultáneos de POS físico y reservas de E-commerce, el servicio de inventario (\texttt{inventory.ts}) utiliza transacciones pesimistas (\texttt{prisma.\$transaction}). Dentro de la transacción, se ejecuta una consulta raw SQL utilizando \texttt{SELECT * FROM "Inventario" ... FOR UPDATE}. Esto bloquea temporalmente las filas de stock específicas de los productos requeridos, forzando a que cualquier transacción concurrente deba esperar a que la transacción actual confirme (\textit{commit}) o aborte (\textit{rollback}).
...
[Inventory-Service] Stock verified. Deducting 2 units from available stock.
